...
class MyPageNumberPagination(PageNumberPagination):
    page_size = 2  #每页显示的个数
    page_size_query_param = 'size' #自定义每页的显示个数 http://127.0.0.1:8000/api/V1/page/?size=个数
    max_page_size = 5   #每页最大的显示个数
    page_query_param = 'page'  #显示第几页内容 http://127.0.0.1:8000/api/V1/page/?page=2


class MyPageNumber2Pagination(LimitOffsetPagination):

    default_limit = 2  #默认限制只显示几个数据
    limit_query_param = "limit"  #限制参数 http://127.0.0.1:8000/api/V1/page/?limit=1
    offset_query_param = "offset" #从第几个开始到最后
    max_limit = 5   #限制了接口最大能取多少个值
...
class PageView(APIView):
    ...
    def get(self,request,*args,**kwargs):
       ...
       #pager_roles = pg.paginate_queryset(queryset=roles,request=request,view=self)
        ...
        #return pg.get_paginated_response(ser.data) #可以在接口中显示上一页面或者下一页面的url
